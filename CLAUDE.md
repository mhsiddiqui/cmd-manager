...
`cmd-manager` is a small Python library that brings Django-style management commands to FastAPI/Flask/etc. It wraps `click` so users can drop `Command` classes into a package and have them auto-discovered as CLI subcommands.
